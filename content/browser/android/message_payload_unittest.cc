...
TEST(MessagePayloadTest, SelfTest_ArrayBuffer) {
  ...
  const auto& array_buffer =
      absl::get<blink::WebMessageArrayBufferPayloadPtr>(generated_message);
  EXPECT_EQ(data.size(), array_buffer->GetLength());
  EXPECT_FALSE(array_buffer->SupportGetArrayBuffer());
  std::vector<uint8_t> copied_data(data.size());
  array_buffer->CopyArrayBufferInto(base::make_span(copied_data));
  ...
  const auto& array_buffer2 =
      absl::get<blink::WebMessageArrayBufferPayloadPtr>(generated_message2);
  ...
  auto generated_message = android::ConvertToWebMessagePayloadFromJava(
      android::ConvertWebMessagePayloadToJava(
          blink::WebMessageArrayBufferPayload::CreateForTesting(
              std::vector<uint8_t>())));
  EXPECT_EQ(absl::get<blink::WebMessageArrayBufferPayloadPtr>(generated_message)
                ->GetLength(),
            0u);
}
